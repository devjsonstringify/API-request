...
var moment = require("moment"),
    ...
    animateSearch = document.querySelector(".searchBox"),
    ...
    if (rejson.data.status == "ok") {
            ...
            if ( rejson.data.articles[i].author != null || rejson.data.articles[i].urlToImage != null || rejson.data.articles[i].description != null ) {
                ...
                newsHtml += '<li class="newsItem">' +
                    ...
                    '<figure class="newsContent">' +
                        '<div class="progressive__bg progressive__img progressive--not-loaded NewsImgWrap" data-progressive=" '+ rejson.data.articles[i].urlToImage +' "  style=" background: url(' + rejson.data.articles[i].urlToImage + ');">' + 
                    ...
                    "<figcaption>" +
                        ...
                        '<p class="newsDesc">' + _.truncate( rejson.data.articles[i].description, { length: 150, omission: " [...]" }) + "</p>" +
                    "</figcaption>" +
                    "</figure>" +
                    '<div class="card-body">' +
                        '<p class="card-link">' + rejson.data.articles[i].source.name + "</p>" +
                        '<p class="card-link publishedTime">' + importFunctions.converTime( rejson.data.articles[i].publishedAt ) +  "</p>" +
                    "</div>" +
                    "</div>" +
              "</li>";
        ...
        newsHtml += "</ul>";
            document.querySelector( ".displayRequestData" ).innerHTML = newsHtml;
             ...
             importFunctions.showSpinner();
             animateSearch.style.width = "auto"; 
                 } //if(rejson.data)
            }
        })
